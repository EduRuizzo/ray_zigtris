...
pub fn build(b: *std.Build) !void {
    ...
    const exe_unit_tests = b.addTest(.{
        .root_source_file = .{ .path = "src/piece.zig" },
        .target = target,
        .optimize = optimize,
    });
    const run_exe_unit_tests = b.addRunArtifact(exe_unit_tests);

    const man_unit_tests = b.addTest(.{
        .root_source_file = .{ .path = "src/manager.zig" },
        .target = target,
        .optimize = optimize,
    });
    const run_man_unit_tests = b.addRunArtifact(man_unit_tests);
    ...
    test_step.dependOn(&run_man_unit_tests.step);
}
